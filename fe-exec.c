...
static int  PQsendQueryInternalThread(PGconn *conn, const char *query, bool newQuery);
static bool PQsendQueryStartThread(PGconn *conn, bool newQuery);
...
PGThreadStatusType threadStatus = PGTHREAD_UNLOCK;
...
static void
pqAppendCmdQueueEntryAsync(PGconn *conn, PGcmdQueueEntry *entry)
{
	Assert(entry->next == NULL);
...
int
PQsendQueryThread(PGconn *conn, const char *query)
{
	return PQsendQueryInternalThread(conn, query, true);
}
...
/*
 * PQexecThread
 *	  implementation of the PQexec in case of splitting the connection 
 *	  between threads.
 */
